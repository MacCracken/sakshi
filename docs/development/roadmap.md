...
_None open._ The atomic ring buffer (the prior v2.3.0 lane) **shipped in
v2.3.0** — `SK_OUT_ATOMIC_RING`, `fetch_add`-reservation MPSC writer,
`sakshi_aring_*` single-reader API, benched at 1.0× the plain ring cost
(no contention). See [`CHANGELOG.md`](../../CHANGELOG.md). Full per-CPU
partitioning is still upstream-blocked (item #4 below).
